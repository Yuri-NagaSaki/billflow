...
**基础URL:** `http://127.0.0.1:8787/api`  
**受保护API基础URL:** `http://127.0.0.1:8787/api/protected`

## 认证机制

除 `/api/auth/*` 外，所有 `/api/*` 接口均需要登录后访问（基于用户名密码的会话认证）。

### 认证流程

1. **登录**: 使用管理员用户名和密码通过 `POST /api/auth/login` 端点获取会话
2. **会话验证**: 会话存储在 D1 的 `sessions` 表中，Cookie 中存储会话ID（`sid`）
...
在 `wrangler.toml` 的 `vars` 或通过 `wrangler secret` 配置以下环境变量：

```bash
# 会话 Cookie 配置（可选）
SESSION_COOKIE_SECURE=auto  # true/false/auto
...
所有API响应均为JSON格式，统一包含 `success` 字段。
...
# 验证登录状态
curl -b cookie.txt http://127.0.0.1:8787/api/auth/me
```

#### 2. 使用会话访问受保护接口
```bash
# 创建订阅
curl -X POST http://127.0.0.1:8787/api/protected/subscriptions \
  -H "Content-Type: application/json" \
  -b cookie.txt \
  -d '{
    "name": "Netflix",
...
# 获取订阅列表
curl -b cookie.txt http://127.0.0.1:8787/api/subscriptions

# 获取月度收入分析
curl -b cookie.txt "http://127.0.0.1:8787/api/analytics/monthly-revenue?start_date=2025-01-01&end_date=2025-12-31&currency=USD"
```
...
#### 发送测试 Telegram 通知
```bash
curl -X POST http://127.0.0.1:8787/api/protected/notifications/test \
  -H "Content-Type: application/json" \
  -b cookie.txt \
...
#### 更新通知设置
```bash
curl -X PUT http://127.0.0.1:8787/api/protected/notifications/settings/1 \
  -H "Content-Type: application/json" \
  -b cookie.txt \
  -d '{
...
### 必需环境变量
数据库使用 D1 绑定（`DB`）配置在 `wrangler.toml`，无需设置 `DATABASE_PATH`。
